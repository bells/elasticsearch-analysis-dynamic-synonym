image_host ?=
plugin_version := $(shell python3 -c 'import xml.etree.ElementTree as E; print(E.parse("pom.xml").getroot().find("{http://maven.apache.org/POM/4.0.0}version").text)')
...
build_image_from_zip: ## Build an image from the verified plugin ZIP without recompiling.
	...
	docker build -f Dockerfile.package -t $(image_name) .
